...
pub fn parse(s: &str) -> Result<SlowType, String> {
    ...
    if !s_left.is_empty() {
        return Err(format!(
            "Type parse() error: extra closeparen\n when parsing: {}",
            s
        ));
    }
    Ok(ty)
...
fn parse_aux(mut s: &str) -> Result<(SlowType, &str), String> {
    ...
    fn finish(mut res: Vec<SlowType>) -> Result<SlowType, String> {
        if res.is_empty() {
            return Err(
                "Type parse() error: unexpected empty parens or empty string in type".into(),
            );
        }
        if res.len() == 1 {
        ...
        match head {
            ...
            SlowType::Term(name, args) => {
                if !args.is_empty() {
                    Err(
                        "Type parse() error: Term type applied to args like ((list int) int)"
                            .into(),
                    )
                } else {
    ...
    loop {
        ...
        let (item, s_new) = s.split_at(s.find(|c| [' ', ')'].contains(&c)).unwrap_or(s.len()));
        s = s_new;
        ...
        if item == arrow {
            ...
            // construct the arrow
            return Ok((
                SlowType::Term(ARROW_SYM.clone(), vec![ty_left, ty_right]),
                s,
            ));
        }
